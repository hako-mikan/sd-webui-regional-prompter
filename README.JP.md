...
## 更新情報 2026.09.04(JST)
- Forge NeoのZ-Image, Anima, Krea2に対応

| モデル \ モード | Latent | Attention | 領域別LoRA |
|---------------|--------|-----------|-------------|
| Z-Image       | ○      | △         | ×           |
| Krea2         | ○      | ○         | ×           |
| Anima         | ○      | ×         | ×           |
...
○：対応  
△：動作しますが、領域が分離するというより影響し合う程度になります。背景や風景の描き分けには有効ですが、各領域がそれぞれ独立した物体を要求する場合は弱くなります。  
×：非対応  

　領域別LoRAはU-Netのブロック名を前提としているため、これらのモデルでは対象が見つからず何も起こりません。
