...
class LoadedModel:
    ...
    def model_load(self, async_kept_memory=-1):
        ...
        if not disable_async_load:
            ...
            for m in self.real_model.modules():
                if hasattr(m, "scripts.ldm_patched_cast_weights"):
                    m.prev_scripts.ldm_patched_cast_weights = m.scripts.ldm_patched_cast_weights
                    m.scripts.ldm_patched_cast_weights = True
    ...
    def model_unload(self, avoid_model_moving=False):
        if self.model_accelerated:
            for m in self.real_model.modules():
                if hasattr(m, "prev_scripts.ldm_patched_cast_weights"):
                    m.scripts.ldm_patched_cast_weights = m.prev_scripts.ldm_patched_cast_weights
                    del m.prev_scripts.ldm_patched_cast_weights
...
def unet_manual_cast(weight_dtype, inference_device):
    ...
    fp16_supported = scripts.ldm_patched.modules.model_management.should_use_fp16(inference_device, prioritize_performance=False)
