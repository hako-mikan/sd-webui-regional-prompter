...
import scripts.ldm_patched.modules.model_detection
import scripts.ldm_patched.modules.model_patcher
import scripts.ldm_patched.modules.ops

import scripts.ldm_patched.controlnet.cldm
import scripts.ldm_patched.t2ia.adapter

from scripts.ldm_patched.modules.ops import main_stream_worker
...
class ControlBase:
    def __init__(self, device=None):
        ...
        if device is None:
            device = scripts.ldm_patched.modules.model_management.get_torch_device()
        self.device = device
...
class ControlNet(ControlBase):
    def __init__(self, control_model, global_average_pooling=False, device=None, load_device=None, manual_cast_dtype=None):
        ...
        self.control_model_wrapped = scripts.ldm_patched.modules.model_patcher.ModelPatcher(self.control_model, load_device=load_device, offload_device=scripts.ldm_patched.modules.model_management.unet_offload_device())
    ...
    def get_control(self, x_noisy, t, cond, batched_number):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * 8 != self.cond_hint.shape[2] or x_noisy.shape[3] * 8 != self.cond_hint.shape[3]:
            if self.cond_hint is not None:
                del self.cond_hint
            self.cond_hint = None
            self.cond_hint = scripts.ldm_patched.modules.utils.common_upscale(self.cond_hint_original, x_noisy.shape[3] * 8, x_noisy.shape[2] * 8, 'nearest-exact', "center").to(dtype)
...
class ControlLoraOps:
    class Linear(torch.nn.Module):
        ...
        def forward(self, input):
            weight, bias, signal = scripts.ldm_patched.modules.ops.cast_bias_weight(self, input)
...
class ControlLora(ControlNet):
    ...
    def pre_run(self, model, percent_to_timestep_function):
        ...
        if self.manual_cast_dtype is None:
            class control_lora_ops(ControlLoraOps, scripts.ldm_patched.modules.ops.disable_weight_init):
                pass
        else:
            class control_lora_ops(ControlLoraOps, scripts.ldm_patched.modules.ops.manual_cast):
                pass
            dtype = self.manual_cast_dtype

        controlnet_config["operations"] = control_lora_ops
        controlnet_config["dtype"] = dtype
        self.control_model = scripts.ldm_patched.controlnet.cldm.ControlNet(**controlnet_config)
        self.control_model.to(scripts.ldm_patched.modules.model_management.get_torch_device())
        ...
        for k in sd:
            weight = sd[k]
            try:
                scripts.ldm_patched.modules.utils.set_attr(self.control_model, k, weight)
            except:
                pass

        for k in self.control_weights:
            if k not in {"lora_controlnet"}:
                scripts.ldm_patched.modules.utils.set_attr(self.control_model, k, self.control_weights[k].to(dtype).to(scripts.ldm_patched.modules.model_management.get_torch_device()))
    ...
    def inference_memory_requirements(self, dtype):
        return scripts.ldm_patched.modules.utils.calculate_parameters(self.control_weights) * scripts.ldm_patched.modules.model_management.dtype_size(dtype) + ControlBase.inference_memory_requirements(self, dtype)

def load_controlnet(ckpt_path, model=None):
    controlnet_data = scripts.ldm_patched.modules.utils.load_torch_file(ckpt_path, safe_load=True)
    if "lora_controlnet" in controlnet_data:
        return ControlLora(controlnet_data)

    controlnet_config = None
    if "controlnet_cond_embedding.conv_in.weight" in controlnet_data: #diffusers format
        unet_dtype = scripts.ldm_patched.modules.model_management.unet_dtype()
        controlnet_config = scripts.ldm_patched.modules.model_detection.unet_config_from_diffusers_unet(controlnet_data, unet_dtype)
        diffusers_keys = scripts.ldm_patched.modules.utils.unet_to_diffusers(controlnet_config)
    ...
    if controlnet_config is None:
        unet_dtype = scripts.ldm_patched.modules.model_management.unet_dtype()
        controlnet_config = scripts.ldm_patched.modules.model_detection.model_config_from_unet(controlnet_data, prefix, unet_dtype, True).unet_config
    load_device = scripts.ldm_patched.modules.model_management.get_torch_device()
    manual_cast_dtype = scripts.ldm_patched.modules.model_management.unet_manual_cast(unet_dtype, load_device)
    if manual_cast_dtype is not None:
        controlnet_config["operations"] = scripts.ldm_patched.modules.ops.manual_cast
    controlnet_config.pop("out_channels")
    controlnet_config["hint_channels"] = controlnet_data["{}input_hint_block.0.weight".format(prefix)].shape[1]
    control_model = scripts.ldm_patched.controlnet.cldm.ControlNet(**controlnet_config)

    if pth:
        if 'difference' in controlnet_data:
            if model is not None:
                scripts.ldm_patched.modules.model_management.load_models_gpu([model])
...
class T2IAdapter(ControlBase):
    ...
    def get_control(self, x_noisy, t, cond, batched_number):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * 8 != self.cond_hint.shape[2] or x_noisy.shape[3] * 8 != self.cond_hint.shape[3]:
            ...
            self.cond_hint = scripts.ldm_patched.modules.utils.common_upscale(self.cond_hint_original, width, height, 'nearest-exact', "center").float()
...
def load_t2i_adapter(t2i_data):
    ...
    if 'adapter.body.0.resnets.0.block1.weight' in t2i_data: #diffusers format
        ...
        t2i_data = scripts.ldm_patched.modules.utils.state_dict_prefix_replace(t2i_data, prefix_replace)
    keys = t2i_data.keys()

    if "body.0.in_conv.weight" in keys:
        cin = t2i_data['body.0.in_conv.weight'].shape[1]
        model_ad = scripts.ldm_patched.t2ia.adapter.Adapter_light(cin=cin, channels=[320, 640, 1280, 1280], nums_rb=4)
        ...
        if cin == 256 or cin == 768:
            xl = True
        model_ad = scripts.ldm_patched.t2ia.adapter.Adapter(cin=cin, channels=[channel, channel*2, channel*4, channel*4][:4], nums_rb=2, ksize=ksize, sk=True, use_conv=use_conv, xl=xl)
    else:
        return None
